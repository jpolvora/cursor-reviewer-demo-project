...
The GitHub Actions workflow [.github/workflows/review.yml](.github/workflows/review.yml) triggers on pull requests targeting the `main` branch.
...
3. Fetches and runs the remote execution script `run.sh` from the `agentic-code-reviewers` repository.
4. Performs an agentic AI review using `cursor-sdk` over the files changed in the PR.
5. Posts actionable review threads directly onto the GitHub Pull Request.
6. An optional **Auto Fix** pipeline ([auto-fix.yml](.github/workflows/auto-fix.yml)) can automatically resolve review threads.
...
| `AGENTIC_CODE_REVIEWERS_GITHUB_TOKEN` | GitHub token with PR read/write permissions for automated workflows. |

*(The `GITHUB_TOKEN` is automatically provided by GitHub Actions for basic authentication).*
...
*   **`.github/workflows/review.yml`** — PR-triggered AI code review pipeline.
